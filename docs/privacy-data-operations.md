...
This document defines implementation requirements for the future system receiving the webhook. It is not legal advice, a certification or a substitute for legal review.

## Provider gate

The provider is intentionally not hardcoded. Its name, privacy role, processing region, transfer safeguards and optional privacy-policy URL come from the `DATA_PROVIDER_*` environment variables. `LEAD_WEBHOOK_URL` contains the server-to-server endpoint.

The API must return HTTP `503` and send nothing unless both the webhook URL and the minimum provider configuration are present. User-facing errors must remain generic and must not disclose environment-variable names, infrastructure details or stack traces.
...
The provider receives one HTTP payload with two explicitly separate sections:
...
Email and consent must never be placed in `researchResponse`. Do not add a shared identifier, IP address, user agent, fingerprint, advertising identifier or other field that unnecessarily links the sections. Do not enrich the research response with a name, exact age, address, documents, payment data or sensitive information.
...
- preserve the webhook timeout and handle provider failures with generic errors;
