  ...
  return (
    <header className="site-header">
      ...
      <div className="container nav">
        <Link className="brand" href={`/${locale}`}><span className="brand-mark" aria-hidden="true">S</span>{businessConfig.brandName}</Link>
        <nav className="nav-links" aria-label={isIt ? "Navigazione principale" : "Main navigation"}>
          ...
          <Link className="locale" href={getAlternatePath(locale, page)} hrefLang={isIt ? "en" : "it-IT"} aria-label={isIt ? "View this page in English" : "Visualizza questa pagina in italiano"}>{isIt ? "EN" : "IT"}</Link>
          <Link className="button" href={`${contact}#availability-form`}>{isIt ? "Richiedi disponibilità" : "Check availability"}</Link>
        </nav>
      </div>
    </header>
  );
